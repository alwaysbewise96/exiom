...
*) ;;
    ...
    #Checking azure before install
    if command -v az > /dev/null 2>&1; then
        echo -e "${check_mark} ${LightCyan}Azure-CLI already installed${Color_Off}"
    else
...
#
echo -e -n "${Green}Please enter your resource group name: (Default 'exiom'), press enter) \n>> ${Color_Off}"
read resource_group

if [[ "$resource_group" == "" ]]; then
    echo -e "${Blue}Selected default option 'exiom'${Color_Off}"
    resource_group="exiom"
fi
...
echo -e -n "${Green}Would you like to configure connection to an exiom Pro Instance? Y/n (Must be deployed.) (optional, default 'n', press enter) \n>> ${Color_Off}"
read ans

if [[ "$ans" == "Y" ]]; then
    echo -e -n "${Green}Enter the exiom pro instance name \n>> ${Color_Off}"
    read appliance_name
