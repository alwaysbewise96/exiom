...
*) ;;
	...
	#Checking aws before install
	if command -v aws > /dev/null 2>&1; then
		echo -e "${check_mark} ${LightCyan}aws already installed${Color_Off}"
	else	
...
function awssetup(){
	...
	echo -e "${BGreen}Creating an exiom Security Group: ${Color_Off}"
	aws ec2 delete-security-group --group-name exiom > /dev/null 2>&1
	sc="$(aws ec2 create-security-group --group-name exiom --description "exiom SG")"
