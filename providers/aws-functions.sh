...
instance_pretty() {
    ...
    type="$(jq -r .default_size "$base_dir/exiom.json")"    
    #getCosts
...
# 
generate_sshconfig() {
accounts=$(ls -l "$base_dir/accounts/" | grep "json" | grep -v 'total ' | awk '{ print $9 }' | sed 's/\.json//g')
current=$(ls -lh ~/.exiom/exiom.json | awk '{ print $11 }' | tr '/' '\n' | grep json | sed 's/\.json//g') > /dev/null 2>&1
sshnew="$base_dir/.sshconfig.new$RANDOM"
droplets="$(instances)"
echo -n "" > $sshnew
echo -e "\tServerAliveInterval 60\n" >> $sshnew
sshkey="$(cat "$base_dir/exiom.json" | jq -r '.sshkey')"
echo -e "IdentityFile $HOME/.ssh/$sshkey" >> $sshnew
generate_sshconfig="$(cat "$base_dir/exiom.json" | jq -r '.generate_sshconfig')"

if [[ "$generate_sshconfig" == "private" ]]; then

 echo -e "Warning your SSH config generation toggle is set to 'Private' for account : $(echo $current)."
 echo -e "exiom will always attempt to SSH into the instances from their private backend network interface. To revert run: exiom-ssh --just-generate"
 ...
 echo -e "exiom will never attempt to regenerate the SSH config. To revert run: exiom-ssh --just-generate"
...
create_instance() {
  ...
  aws ec2 run-instances --image-id "$image_id" --count 1 --instance-type "$size" --region "$region" --security-groups exiom --tag-specifications "ResourceType=instance,Tags=[{Key=Name,Value=$name}]" 2>&1 >> /dev/null
...
conf_check() {
	...
	l="$(cat "$base_dir/.sshconfig" | grep "$instance" | wc -l | awk '{ print $1 }')"
