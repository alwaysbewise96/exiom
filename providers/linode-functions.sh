...
instance_pretty() {
  ...
  type="$(jq -r .default_size "$base_dir/axiom.json")"
...
generate_sshconfig() {
	...
	current=$(ls -lh "$base_dir/axiom.json" | awk '{ print $11 }' | tr '/' '\n' | grep json | sed 's/\.json//g') > /dev/null 2>&1
	droplets="$(instances)"
    sshnew="$base_dir/.sshconfig.new$RANDOM"
	echo -n "" > $sshnew 
	echo -e "\tServerAliveInterval 60\n" >> $sshnew 
	sshkey="$(cat "$base_dir/axiom.json" | jq -r '.sshkey')"
	echo -e "IdentityFile $HOME/.ssh/$sshkey" >> $sshnew 
	generate_sshconfig="$(cat "$base_dir/axiom.json" | jq -r '.generate_sshconfig')"
    ...
    mv $sshnew  $base_dir/.sshconfig
    	...
    	echo -e "axiom will never attempt to regenerate the SSH config. To change edit $HOME/exiom/account/$current.json"
...
create_instance() {
	...
	region="$4"
	boot_script="$5"
	root_pass="$(jq -r .do_key "$base_dir/axiom.json")"
